...
const expandableText = document.getElementById("expandable-text"),
  readMoreButton = document.getElementById("read-more-button");
let expanded = !1;
...
function changeTextRandomly() {
  const e = Math.floor(Math.random() * textArray.length);
  changingTextElement.textContent = textArray[e];
}
function openLink(e) {
  const t = e.replace(".html", "");
  return window.open(t, "_blank"), !1;
}
(animationInterval = setInterval(changeTextRandomly, 1e9)),
  changingTextElement.addEventListener("mouseenter", () => {
    clearInterval(animationInterval);
  }),
  changingTextElement.addEventListener("mouseleave", () => {
    animationInterval = setInterval(changeTextRandomly, 1e3);
  });
